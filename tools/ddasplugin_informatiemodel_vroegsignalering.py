...
SIGNAALPARTNER_ID = "EAID_3643CF44_EFAA_4939_9AEB_ACA8D8EE11F9" #{3643CF44-EFAA-4939-9AEB-ACA8D8EE11F9}
...
class DDASPluginInformatiemodel(Plugin):
    def transformLogic(self, args, root_package, schema_from, schema_to):
        logger.info("Starting DDAS-Plugin to create the informatiemodel voor Vroegsignalering.")
        # Clean up
        schema_to.clean()
        ...
        if not root_package.id == VROEGSIGNALERING_PACKAGE_ID:
            msg = (
                ...
                f" vroegsignalering with id {VROEGSIGNALERING_PACKAGE_ID}."
            )
            logger.error(msg)
        ...
        set_org(SIGNAALPARTNER_ID, ["(statutaire) naam", "kvk-nummer"])
        #org = schema_to.get_class(SIGNAALPARTNER_ID)
        #org.attributes.append(
        #    Attribute(
        #        id=util.getEAGuid(),
        #        name="gemeentecode",
        ...
        logger.info("DDAS Informatiemodel Plugin voor Vroegsignalering finished.")
